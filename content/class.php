...
class barang {
    ...
    public function showBrg()
    {
		...
		echo "<form method='post' action='content/ctkbrg.php'>"
									."<button type='submit' class='btn btn-black' formtarget='_blank'>Cetak</button></td>"
									."</form>";
...
class distribusi
{
    ...
    public function detailD($a)
    {
		...
		//echo "<form method='post' action='?p'>"
			//						."<button type='submit' name='$a' class='btn btn-black' formtarget='_blank'>Cetak</button></td>"
				//					."</form>";
		echo "</div></div></div></div>";
...
class detailDistribusi
{
	...
	public function ctkDetD($a)
    {
        $db = new database();
        $db->dbOpen();
        $sql="select distribusi.idDis,distribusi.idKamar,distribusi.idTek,teknisi.namaTek,detaildis.id,barang.namaBrg,detaildis.jumlah from distribusi,detaildis,kamar,barang,teknisi where distribusi.idDis=detaildis.idDis and distribusi.idKamar=kamar.idKamar and barang.id=detaildis.id and teknisi.idTek=distribusi.idTek and distribusi.idDis = '$'";
        $result=mysql_query($sql);
        
		echo "ID Distribusi = $a";
		echo "<tr><td>Nama Barang</td><td>Jumlah</td></tr>";
		while ($data = mysql_fetch_array($result)) {
